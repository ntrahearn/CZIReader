...
/**
 * ZeissCZIReaderX is the file format reader for Zeiss .czi files.
 * This is a modified version of ZeissCZIReader from BioFormats 6.0.1
 */
public class ZeissCZIReaderX extends FormatReader {
  ...
  protected List<ExtendedCoreMetadata> excore; //Added by NTrahearn

  // -- Constructor --

  /** Constructs a new Zeiss .czi reader. */
  public ZeissCZIReaderX() {
    super("Zeiss CZI", "czi");
  ...
  protected void initFile(String id) throws FormatException, IOException {
    ...
    //Added by NTrahearn
    excore = new ArrayList<ExtendedCoreMetadata>();
    
    for (int i=0; i<core.size(); i++) {
        excore.add(new ExtendedCoreMetadata());
    }
      ...
      for (int s=0; s<core.size();) {
        ...
        if (originalMosaicCount > 1) {
          ...
          //if (core.get(s).sizeX == x && core.get(s).sizeY == y) {
            ...
            excore.get(s).xPosition = minCol; //Added by NTrahearn
            excore.get(s).yPosition = minRow; //Added by NTrahearn
          //}
        }
        ...
        for (int r=0; r<core.get(s).resolutionCount; r++) {
          ...
          if (!hasValidPlane && r > 0 && !keepMissingPyramid) {
            ...
            excore.get(s + r).xPosition = excore.get(s).xPosition/div; //Added by NTrahearn
            excore.get(s + r).yPosition = excore.get(s).yPosition/div; //Added by NTrahearn
          }
  ...
  // -- Things I added (NTRAHEARN) --
  public int getPositionX() {
    FormatTools.assertId(currentId, true, 1);
    return excore.get(getCoreIndex()).xPosition;
  }

  public int getPositionY() {
    FormatTools.assertId(currentId, true, 1);
    return excore.get(getCoreIndex()).yPosition;
  }
    ...
    for (Segment segment : segments) {
      if (segment instanceof Attachment) {
          ...
          (name.equals("SlidePreview") && !foundPreview))
        {
          ...
          ZeissCZIReaderX thumbReader = new ZeissCZIReaderX();
      ...
      for (DimensionEntry dimension : plane.directoryEntry.dimensionEntries) {
        ...
        switch (dimension.dimension.charAt(0)) {
          case 'X':
            ...
            if (allowAutostitching() || dimension.size > ms0.sizeX || dimension.size == dimension.storedSize) {
          ...
          case 'Y':
            ...
            if (allowAutostitching() || dimension.size > ms0.sizeY || dimension.size == dimension.storedSize) {
